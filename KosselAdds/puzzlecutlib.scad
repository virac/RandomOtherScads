...
module xMaleNegCut(offset = 0, cut = xCut1)
{
	difference()
	{
		children(0);	
		rotate ([0,0,180]) translate([0,-offset,0]) makePuzzleStamp(cutLocations = cut);
	}
}
...
module xFemaleNegCut(offset = 0, cut = xCut1)
{
	intersection()
	{
		children(0);
...
{
	cut_offset = max(cutSize/2,cutSize/3+2.5);
	difference()
	{
		//make the cube
		...
		{
			translate([i,0,0])
				cube ([(cutSize / 2) - kerf * 2, cut_offset*2 - kerf * 2,stampSize[2]], center = true);
			translate([i,cut_offset,0])
